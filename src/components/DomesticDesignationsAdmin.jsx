...
const DomesticDesignationsAdmin = ({ organizationId }) => {
  ...
  const containerStyle = { padding: '24px', backgroundColor: '#0f1419', minHeight: '100vh' };
  const warningBannerStyle = { backgroundColor: '#d97706', border: '1px solid #f59e0b', borderRadius: '8px', padding: '16px', marginBottom: '24px', color: '#ffffff', fontSize: '14px', fontWeight: '600', lineHeight: '1.5' };
  const headerStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' };
  const titleStyle = { fontSize: '28px', fontWeight: 'bold', color: '#ffd700' };
  const buttonStyle = { backgroundColor: '#ffd700', color: '#0f1419', border: 'none', padding: '10px 16px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '14px' };
  const filterBarStyle = { display: 'flex', gap: '16px', marginBottom: '24px', alignItems: 'center', flexWrap: 'wrap' };
  const filterStyle = { backgroundColor: '#1a202c', color: '#ffffff', border: '1px solid #ffd700', padding: '8px 12px', borderRadius: '6px', fontSize: '14px', cursor: 'pointer' };
  const searchStyle = { flex: 1, minWidth: '200px', backgroundColor: '#1a202c', color: '#ffffff', border: '1px solid #ffd700', padding: '8px 12px', borderRadius: '6px', fontSize: '14px' };
  const tableStyle = { width: '100%', borderCollapse: 'collapse', backgroundColor: '#1a202c', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 4px 6px rgba(0,0,0,0.3)' };
  const theadStyle = { backgroundColor: '#2d3748', borderBottom: '2px solid #ffd700' };
  const thStyle = { padding: '12px 16px', textAlign: 'left', color: '#ffd700', fontWeight: 'bold', fontSize: '13px' };
  const tbodyTrStyle = { borderBottom: '1px solid #374151' };
  const tdStyle = { padding: '12px 16px', color: '#e5e7eb', fontSize: '13px' };
  const actionButtonStyle = { backgroundColor: '#ffd700', color: '#0f1419', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px', marginRight: '6px' };
  const secondaryButtonStyle = { backgroundColor: '#6b7280', color: '#ffffff', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px', marginRight: '6px' };
  ...
  const modalStyle = { backgroundColor: '#1a202c', border: '2px solid #ffd700', borderRadius: '8px', padding: '24px', maxWidth: '600px', width: '90%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 25px rgba(0,0,0,0.5)' };
  const formGroupStyle = { marginBottom: '16px' };
  const labelStyle = { display: 'block', color: '#ffd700', fontWeight: 'bold', marginBottom: '6px', fontSize: '14px' };
  const inputStyle = { width: '100%', backgroundColor: '#0f1419', color: '#ffffff', border: '1px solid #ffd700', padding: '10px 12px', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' };
  const textareaStyle = { ...inputStyle, minHeight: '80px', fontFamily: 'inherit', resize: 'vertical' };
  const formButtonGroupStyle = { display: 'flex', gap: '12px', marginTop: '24px' };
  const messageStyle = (type) => ({ padding: '12px 16px', borderRadius: '6px', marginBottom: '16px', fontSize: '14px', fontWeight: '500', backgroundColor: type === 'error' ? '#dc2626' : '#059669', color: '#ffffff' });

  const statusBadgeStyle = (status) => {
    const map = { active: '#10b981', delisted: '#ef4444', under_review: '#f59e0b' };
    return { display: 'inline-block', backgroundColor: map[status] || '#374151', color: '#ffffff', padding: '4px 8px', borderRadius: '4px', fontSize: '12px', fontWeight: '600' };
  };
          ...
          <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={formGroupStyle}>
                ...
                <select style={inputStyle} name="status" value={formData.status} onChange={handleInputChange} required>
                  <option value="active">Active</option>
                  <option value="delisted">Delisted</option>
                  <option value="under_review">Under Review</option>
                </select>
              ...
              <div style={formGroupStyle}>
                ...
                <textarea style={textareaStyle} name="notes" value={formData.notes} onChange={handleInputChange} placeholder="Additional information or context..." />
              </div>
              <div style={formButtonGroupStyle}>
                ...
                <button type="button" style={{ ...buttonStyle, backgroundColor: '#6b7280', color: '#ffffff' }} onClick={resetForm}
