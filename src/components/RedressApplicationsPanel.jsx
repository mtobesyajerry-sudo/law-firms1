...
const RedressApplicationsPanel = ({ matchId }) => {
  ...
  const containerStyle = {
    backgroundColor: dashboardStyles.colors.darkNavy,
  ...
  const sectionTitleStyle = {
    ...
    marginBottom: '16px',
    marginTop: '24px',
    borderBottom: `2px solid ${dashboardStyles.colors.gold}`,
    paddingBottom: '8px',
  };

  const applicationCardStyle = {
    backgroundColor: dashboardStyles.colors.darkGray,
  ...
  const buttonStyle = {
    ...
    color: dashboardStyles.colors.darkNavy,
    border: 'none',
    padding: '10px 20px',
    borderRadius: '4px',
    fontSize: '14px',
  ...
  const secondaryButtonStyle = {
    ...
    color: dashboardStyles.colors.gold,
    border: `2px solid ${dashboardStyles.colors.gold}`,
    padding: '8px 16px',
    borderRadius: '4px',
    fontSize: '14px',
    fontWeight: '600',
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={warningBannerStyle}>
        ⚠️ IMPORTANT: Granted status does NOT automatically lift the TFS freeze. A separate administrative process with the FIU is required.
      </div>
      ...
      <div>
        ...
        {loading ? (
          <p style={{ color: dashboardStyles.colors.lightText }}>Loading applications...</p>
        ) : applications.length === 0 ? (
          <p style={{ color: dashboardStyles.colors.lightText }}>No redress applications for this match yet.</p>
        ) : (
          applications.map(app => (
            <div key={app.id} style={applicationCardStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '12px' }}>
                <div>
                  <h3 style={{ margin: '0 0 4px 0', color: dashboardStyles.colors.gold }}>
                    {app.applicant_name}
                  </h3>
                  <p style={{ margin: '0', fontSize: '12px', color: dashboardStyles.colors.lightText }}>
              ...
              <div style={{ marginBottom: '12px' }}>
                ...
                <p style={{ margin: '0', fontSize: '14px', wordBreak: 'break-word' }}>
                  {app.applicant_contact || 'Not provided'}
                </p>
              </div>
              ...
              {editingId === app.id ? (
                <div style={{ marginTop: '12px', padding: '12px', backgroundColor: dashboardStyles.colors.darkNavy, borderRadius: '4px' }}>
                  <label style={labelStyle}>Update Status</label>
                  <select
                    value={editingStatus || app.status}
                  ...
                  <button
                    ...
                    onMouseLeave={(e) => e.target.style.backgroundColor = dashboardStyles.colors.gold}
                  >
                    Save Status
                  </button>
                  <button
      ...
      <div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            onChange={handleInputChange}
            placeholder="Email, phone, or mailing address"
            style={inputStyle}
          />

          <label style={labelStyle}>Grounds for Redress *</label>
          <textarea
            name="grounds_for_redress"
            value={formData.grounds_for_redress}
            onChange={handleInputChange}
            placeholder="Detailed explanation of why the TFS freeze should be lifted"
            style={textareaStyle}
            required
          />
          ...
          <input
            type="text"
            name="supporting_documents_reference"
            value={formData.supporting_documents_reference}
            onChange={handleInputChange}
            placeholder="File names, URLs, or case numbers of supporting documents"
            style={inputStyle}
          />
          ...
          <button
            ...
            onMouseLeave={(e) => !submitting && (e.target.style.backgroundColor = dashboardStyles.colors.gold)}
          >
