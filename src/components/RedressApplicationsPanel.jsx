...
const RedressApplicationsPanel = ({ matchId }) => {
  ...
  useEffect(() => { fetchApplications(); }, [matchId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('redress_applications').insert([{
        match_id: matchId,
  ...
  const handleStatusUpdate = async (applicationId) => {
    ...
    try {
      ...
      const { data: sessionData } = await supabase.auth.getSession();
      const userId = sessionData?.session?.user?.id;
      const { error: updateError } = await supabase.from('redress_applications')
  ...
  const statusBadgeStyle = (status) => {
    const map = { pending: { bg: '#f57c00', text: '#fff' }, under_review: { bg: '#1976d2', text: '#fff' }, granted: { bg: '#388e3c', text: '#fff' }, denied: { bg: '#d32f2f', text: '#fff' } };
          ...
          applications.map(app => (
            <div key={app.id} style={applicationCardStyle}>
              ...
              {editingId === app.id ? (
                ...
                <button onClick={() => { setEditingId(app.id); setEditingStatus(app.status); }} style={secondaryButtonStyle}>Update Status</button>
              )}
            </div>
          ))
        )}
