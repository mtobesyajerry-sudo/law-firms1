      ...
      <div style={dashboardStyles.contentCard}>
        <div style={dashboardStyles.tabContainer}>
          ...
          <button
            ...
            style={activeTab === 'clients' ? dashboardStyles.tabActive : dashboardStyles.tab}
          >
            Clients ({clients.length})
          </button>
          <button
            onClick={() => setActiveTab('reports')}
            style={activeTab === 'reports' ? dashboardStyles.tabActive : dashboardStyles.tab}
          >
            Risk Reports
          </button>
          <button
          ...
          <button
            ...
            style={activeTab === 'org-users' ? dashboardStyles.tabActive : dashboardStyles.tab}
          >
            Organization Users
          </button>
          <button
            onClick={() => setActiveTab('approvals')}
            style={activeTab === 'approvals' ? dashboardStyles.tabActive : dashboardStyles.tab}
          >
            Role Approvals
          </button>
        </div>
        ...
        {activeTab === 'clients' && (
          <div style={styles.tabContent}>
            ...
            <p style={{ color: '#718096', marginBottom: '24px', fontSize: '14px' }}>
              Total Clients: {clients.length} across {organizations.length} organizations
            </p>
                ...
                <div key={org.id} style={styles.orgAssessmentCard}>
                  <div style={styles.orgAssessmentHeader}>
                    <div>
                      <h3 style={styles.orgAssessmentTitle}>{org.name}</h3>
                      <p style={styles.orgAssessmentSubtitle}>
                        {org.business_type} • {orgClients.length} client{orgClients.length !== 1 ? 's' : ''}
                      </p>
                    </div>
                    <div style={styles.orgAssessmentStats}>
                      <span style={{
                        ...
                        {orgClients.filter(c => c.client_type === 'individual').length} Individual
                      </span>
                      <span style={{
                        ...styles.badge,
                        background: '#e0e7ff',
                        ...
                        {orgClients.filter(c => c.client_type === 'corporate').length} Corporate
                      </span>
                    </div>
                  </div>
                    ...
                    <table style={styles.table}>
                      <thead>
                        <tr>
                          ...
                          <th style={styles.th}>Due Diligence</th>
                          <th style={styles.th}>Status</th>
                      ...
                      <tbody>
                        {orgClients.map((client) => (
                          <tr key={client.id} style={styles.tr}>
                            <td style={styles.td}>
                              <div>
                                <div style={{ fontWeight: '500' }}>{client.full_name || client.entity_name || 'N/A'}</div>
                                {client.email && (
                                  <div style={{ fontSize: '12px', color: '#718096' }}>{client.email}</div>
                                )}
                              </div>
                            </td>
                            <td style={styles.td}>
                              <span style={{
                                ...styles.badge,
                                background: client.client_type === 'individual' ? '#dbeafe' : '#e0e7ff',
                                color: client.client_type === 'individual' ? '#1e40af' : '#4338ca'
                              }}>
                            ...
                            <td style={styles.td}>
                              {client.risk_rating ? (
                                <span style={{
                                  ...styles.badge,
                                  background: getRiskColor(client.risk_rating) + '20',
                                  color: getRiskColor(client.risk_rating)
                                }}>
                                  {getRiskLabel(client.risk_rating)}
                                </span>
                              ) : '-'}
                            </td>
                            <td style={styles.td}>
                              <span style={{
                                ...
                                {client.current_dd_level || 'Not Set'}
                              </span>
                            </td>
                            <td style={styles.td}>
                              <span style={{
                                ...styles.badge,
                                background: client.onboarding_status === 'approved' ? '#d1fae5' :
                                          client.onboarding_status === 'rejected' ? '#fee2e2' : '#fef3c7',
                                color: client.onboarding_status === 'approved' ? '#065f46' :
                                       client.onboarding_status === 'rejected' ? '#991b1b' : '#92400e'
                              }}>
            ...
            {clients.length === 0 && (
              <p style={styles.emptyState}>No clients found.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'reports' && (
          <div style={styles.tabContent}>
            <h2 style={styles.sectionTitle}>Risk Assessment Reports</h2>
            <p style={{ color: '#718096', marginBottom: '24px', fontSize: '14px' }}>
              Comprehensive view of all institutional risk assessments and client risk profiles across all organizations
            </p>

            {organizations.map((org) => {
              const orgAssessments = assessments.filter(a => a.organization_id === org.id && a.status === 'completed');
              const orgClients = clients.filter(c => c.organization_id === org.id);

              if (orgAssessments.length === 0 && orgClients.length === 0) return null;

              const latestAssessment = orgAssessments.length > 0 ? orgAssessments[0] : null;
              const highRiskClients = orgClients.filter(c => c.risk_rating === 'High' || c.risk_rating === 'high').length;
              const mediumRiskClients = orgClients.filter(c => c.risk_rating === 'Medium' || c.risk_rating === 'medium').length;
              const lowRiskClients = orgClients.filter(c => c.risk_rating === 'Low' || c.risk_rating === 'low').length;
                ...
                <div key={org.id} style={styles.orgAssessmentCard}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px', marginBottom: '24px' }}>
                    <div style={{
                      ...
                      <div style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Institutional Risk</div>
                      {latestAssessment ? (
                        <>
                          <div style={{ fontSize: '24px', fontWeight: 'bold', color: getRiskColor(latestAssessment.overall_risk_rating) }}>
                            {getRiskLabel(latestAssessment.overall_risk_rating)}
                          </div>
                        ...
                        <div style={{ fontSize: '14px', color: '#9ca3af' }}>No assessment completed</div>
                      )}
                    </div>
                    ...
                    <div style={{
                      ...
                      <div style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Client Risk Distribution</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    {latestAssessment && (
                      <div style={{
                        ...
                        <div style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Module Scores</div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          {latestAssessment.module_1_score && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                              <span>Module 1 (TC)</span>
                              <span style={{ fontWeight: '500' }}>{latestAssessment.module_1_score.toFixed(1)}/5.0</span>
                            </div>
                          )}
                          {latestAssessment.module_2_score && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                              <span>Module 2 (E)</span>
                              <span style={{ fontWeight: '500' }}>{latestAssessment.module_2_score.toFixed(1)}/5.0</span>
                            </div>
                          )}
                          {latestAssessment.module_3_score && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                              <span>Module 3 (TC)</span>
                              <span style={{ fontWeight: '500' }}>{latestAssessment.module_3_score.toFixed(1)}/5.0</span>
                            </div>
                          )}
                          {latestAssessment.module_4_score && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                              <span>Module 4 (E)</span>
                              <span style={{ fontWeight: '500' }}>{latestAssessment.module_4_score.toFixed(1)}/5.0</span>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div style={styles.tableContainer}>
                        <table style={styles.table}>
                          ...
                          <tbody>
                            {orgClients
                              .filter(c => c.risk_rating === 'High' || c.risk_rating === 'high')
                              .slice(0, 5)
                              .map((client) => (
                                <tr key={client.id} style={styles.tr}>
                                  <td style={styles.td}>
                                  ...
                                  <td style={styles.td}>
                                    {client.last_review_date ? new Date(client.last_review_date).toLocaleDateString() : '-'}
                                  </td>
                                </tr>
                              ))}
                          </tbody>
                      ...
                      {highRiskClients > 5 && (
                        <p style={{ fontSize: '12px', color: '#6b7280', marginTop: '8px', fontStyle: 'italic' }}>
                          Showing 5 of {highRiskClients} high-risk clients
                        </p>
                      )}
                      {highRiskClients === 0 && (
                        <p style={{ fontSize: '14px', color: '#059669', padding: '16px', textAlign: 'center', background: '#d1fae5', borderRadius: '6px' }}>
            ...
            {assessments.length === 0 && clients.length === 0 && (
              <p style={styles.emptyState}>No risk data available.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'org-users' && (
          <OrganizationUserManagement user={user} />
        )}

        {activeTab === 'approvals' && (
          <div style={styles.tabContent}>
            <h2 style={styles.sectionTitle}>Role Upgrade Approvals</h2>
            <p style={{ color: '#718096', marginBottom: '24px', fontSize: '14px' }}>
              View role upgrade requests from all organizations. The dual-approval workflow requires 2 out of 3 authorized users per organization.
            </p>
            <div style={{
              padding: '20px',
              background: '#fef3c7',
              ...
              Note: This view shows all requests across organizations. Individual organization users will see only their organization's requests through the Client Dashboard.
            </div>
            {organizations.map(org => (
              <div key={org.id} style={{ marginBottom: '32px' }}>
                <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#0a1929', marginBottom: '16px' }}>
                  {org.name}
                </h3>
                <DualApprovalInterface user={user} organizationId={org.id} />
              </div>
            ))}
          </div>
        )}
