-- Drop and recreate kyc_clients_decrypted view without enhanced_dd_required column
...
SELECT id,
    ...
    CASE WHEN beneficiaries_encrypted IS NOT NULL THEN decrypt_pii(beneficiaries_encrypted)::jsonb ELSE beneficiaries END AS beneficiaries,
    CASE WHEN beneficial_owners_encrypted IS NOT NULL THEN decrypt_pii(beneficial_owners_encrypted)::jsonb ELSE beneficial_owners END AS beneficial_owners,
...
-- Drop the now-unused enhanced_dd_required column
ALTER TABLE kyc_clients DROP COLUMN IF EXISTS enhanced_dd_required;